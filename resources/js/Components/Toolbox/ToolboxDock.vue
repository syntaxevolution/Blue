...
<template>
    <div class="fixed bottom-4 right-4 z-40 flex flex-col items-end gap-2 font-mono">
        ...
        >
            ...
            <div class="text-[11px] text-amber-300/80 mt-1">
                Click a drill point on the 5×5 grid to plant. Press Esc or tap below to cancel.
            </div>
            <button
                type="button"
        ...
        <div
            v-if="open"
            class="w-72 max-h-[60vh] overflow-y-auto rounded-lg border-2 border-amber-500/40 bg-zinc-900/95 backdrop-blur shadow-2xl shadow-amber-900/20"
        >
        ...
        <button
            type="button"
            class="rounded-full border-2 border-amber-500/60 bg-zinc-900/95 backdrop-blur px-4 py-3 shadow-xl shadow-amber-900/20 text-amber-400 hover:border-amber-400 hover:text-amber-300 transition flex items-center gap-2"
        ...
        >
            ...
            <span class="text-[11px] uppercase tracking-widest font-bold">Toolbox</span>
            <span
                v-if="totalItems > 0"
                class="inline-block rounded-full bg-amber-500 text-zinc-950 text-[10px] font-bold px-1.5 py-0.5"
            >
                {{ totalItems }}
            </span>
        </button>
